...
const getTwitchTags = async (broadcaster_id) => {
  const tagsData = await twitchRequest(`https://api.twitch.tv/helix/streams/tags?broadcaster_id=${broadcaster_id}`);

  return tagsData.data.map((i) => ({ name: i.localization_names['en-us'] }));
};

const getTwitchChannel = async (broadcaster_id) => {
  const channelData = await twitchRequest(`https://api.twitch.tv/helix/channels?broadcaster_id=${broadcaster_id}`);
...
const removeSubscription = async (subscription_id) => {
  ...
  try {
    const response = await axios.delete(`https://api.twitch.tv/helix/eventsub/subscriptions?id=${subscription_id}`, {
      headers: {
        'Client-Id': clientId,
        Authorization: `Bearer ${token}`,
      },
