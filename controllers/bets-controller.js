...
const createBet = async (req, res, next) => {
  ...
  try {
    ...
    console.debug(LOG_TAG, {
      marketQuestion, hot, outcomes, endDate, event: eventId, creator: req.user.id, slug,
    });
...
const betHistory = async (req, res, next) => {
  ...
  try {
    ...
    const interactionsList = await eventService.combineBetInteractions(bet, direction, rangeType, rangeValue);
